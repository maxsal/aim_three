...
ms::libri(ms, qs, data.table, tidyverse, ggcorrplot, visdat, naniar,
    labelled, patchwork, mice, glue)

outcome <- "EM_202.2"

# data -------------------------------------------------------------------------
demo         <- qread(glue("data/private/demo_comb_labs_ehr_20230322_{outcome}.qs"))
...
ggsave(
    glue("results/correlation_plots_{outcome}.pdf"),
    patched,
    width = 14,
    height = 5.5
)
...
ggsave(
    glue("results/missingness_plot_{outcome}.pdf"),
    missing_plot,
    width = 6,
    height = 7.5
)
...
qsave(
    analytic_full,
    glue("data/private/analytic_full_{outcome}.qs")
)

qsave(
    analytic_sub,
    glue("data/private/analytic_sub_{outcome}.qs")
)
