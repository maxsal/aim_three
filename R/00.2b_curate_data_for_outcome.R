...
option_list <- list(
    ...
    make_option(c("-o", "--outcome"),
        type = "character", default = "EM_202.2",
        help = "Phecode corresponding to outcome [default = %default]"
    )
)
...
case_index <- qread(glue("data/private/mgi/20220822/MGI_{outcome}_INDEX_20220822.qs"))
ehr <- qread(glue("data/private/mgi/20220822/ehr_20220822_{outcome}.qs"))

# DEMOGRAPHICS DATA ------------------------------------------------------------
cli_progress_step("Demographics")
demo <- fread(paste0(path, "Demographics_", as.Date(version, "%Y%m%d"), ".txt.gz")) |>
    clean_names()
setnames(
    demo,
...
qsave(
    x        = demo,
    file     = glue("data/private/mgi_demo_{version}_{outcome}.qs"),
    nthreads = 4
)

# SOCIAL HISTORY ---------------------------------------------------------------
cli_progress_step("Smoking and alcohol")
social <- fread(paste0(path, "SocialHx_", as.Date(version, "%Y%m%d"), ".txt.gz")) |>
    clean_names()
setnames(
    social,
    old = c("de_id_patient_id"),
...
social <- social[id %in% ehr[, id], ]

social <- merge.data.table(
    social,
    case_index,
    by = "id", all.x = TRUE
)
social$index_dsb[is.na(social$index_dsb)] <- Inf
social <- social |>
    dplyr::filter(days_since_birth < index_dsb)
...
qsave(
    x        = smoking,
    file     = glue("data/private/mgi_smoking_{version}_{outcome}.qs"),
    nthreads = 4
)
qsave(
    x        = alcohol,
    file     = glue("data/private/mgi_alcohol_{version}_{outcome}.qs"),
    nthreads = 4
)
qsave(
    x        = social,
    file     = glue("data/private/mgi_social_{version}_{outcome}.qs"),
    nthreads = 4
)
...
anthro <- fread(paste0(path, "Anthropometrics_2023-03-22.txt.gz")) |>
    clean_names()
setnames(
    anthro,
    old = c("de_id_patient_id"),
...
anthro <- anthro[id %in% ehr[, id], ]

anthro <- merge.data.table(
    anthro,
    case_index,
    by = "id", all.x = TRUE
)
anthro$index_dsb[is.na(anthro$index_dsb)] <- Inf
anthro <- anthro |>
...
qsave(
    x        = anthro,
    file     = glue("data/private/mgi_anthro_{version}_{outcome}.qs"),
    nthreads = 4
)
qsave(
    x        = ht_wt,
    file     = glue("data/private/mgi_ht_wt_{version}_{outcome}.qs"),
    nthreads = 4
)
...
vitals <- fread(paste0(path, "EncounterVitals_", as.Date(version, "%Y%m%d"), ".txt.gz"),
...
vitals <- vitals[id %in% ehr[, id], ]

vitals <- merge.data.table(
    vitals,
    case_index,
    by = "id", all.x = TRUE
)
vitals$index_dsb[is.na(vitals$index_dsb)] <- Inf
vitals <- vitals |>
    dplyr::filter(dsb < index_dsb)
...
qsave(
    x        = bp,
    file     = glue("data/private/mgi_vitals_{version}_{outcome}.qs"),
    nthreads = 4
)
...
qsave(
    x        = demo_combined,
    file     = glue("data/private/mgi_demo_comb_{version}_{outcome}.qs"),
    nthreads = 4
)
...
labs_results_file <- paste0(path, "LabResults_", as.Date(version, "%Y%m%d"), ".txt.gz")
...
labs <- labs[id %in% ehr[, id], ]

labs <- merge.data.table(
    labs,
    case_index,
    by = "id", all.x = TRUE
)
labs$index_dsb[is.na(labs$index_dsb)] <- Inf
labs <- labs |>
    dplyr::filter(collection_dsb < index_dsb)
...
qsave(
    x        = cleaned_labs,
    file     = glue("data/private/mgi_labs_cleaned_{version}_{outcome}.qs"),
    nthreads = 4
)
...
qsave(
    x        = cleaned_labs_med,
    file     = glue("data/private/mgi_labs_cleaned_medians_{version}_{outcome}.qs"),
    nthreads = 4
)
...
fwrite(
    ...
    file = glue("data/private/mgi_labs_cleaning_summary_{version}_{outcome}.csv")
)
...
qsave(
    x        = demo_labs,
    file     = glue("data/private/demo_comb_labs_{version}_{outcome}.qs"),
    nthreads = 4
)

demo_labs_ehr <- full_join(
    demo_labs,
    ehr,
    by = "id"
)
...
qsave(
    x        = demo_labs_ehr,
    file     = glue("data/private/demo_comb_labs_ehr_{version}_{outcome}.qs"),
    nthreads = 4
)
